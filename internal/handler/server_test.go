...
func TestWebRegisterLoginAndAttendanceFlow(t *testing.T) {
	...
	server, err := NewServer(auth, attendance, service.NewUnitDTService(store, location, nowFunc), service.NewProduksiService(store, location, nowFunc), service.NewOverviewService(store, location, nowFunc), webSessions, location, nowFunc, 2*1024*1024, photo.MaxOutputChars)
	if err != nil {
...
func TestLoginFailureUsesGenericMessage(t *testing.T) {
	...
	server, err := NewServer(auth, attendance, service.NewUnitDTService(store, time.Local, func() time.Time { return now }), service.NewProduksiService(store, time.Local, func() time.Time { return now }), service.NewOverviewService(store, time.Local, func() time.Time { return now }), webSessions, time.Local, func() time.Time { return now }, 2*1024*1024, photo.MaxOutputChars)
	if err != nil {
		t.Fatalf("new server: %v", err)
	}
