...
func TestWebRegisterLoginAndAttendanceFlow(t *testing.T) {
	...
	server, err := NewServer(auth, attendance, webSessions, location, nowFunc, 2*1024*1024, photo.MaxOutputChars)
	if err != nil {
...
func TestLoginFailureUsesGenericMessage(t *testing.T) {
	...
	server, err := NewServer(auth, attendance, webSessions, time.Local, func() time.Time { return now }, 2*1024*1024, photo.MaxOutputChars)
	if err != nil {
		t.Fatalf("new server: %v", err)
	}
